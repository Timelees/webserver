...
template <typename T>
ThreadPool<T>::ThreadPool(int actor_mode, int concurrent_mode, SQLConnectionPool *sql_conn_pool, int thread_num, int max_requests)
    :actor_mode_(actor_mode), concurrent_mode_(concurrent_mode), thread_num_(thread_num), sql_conn_pool_(sql_conn_pool), max_requests_(max_requests), threads_(NULL)
{
...
void ThreadPool<T>::run_HS_HA(){
    std::cout << "[ThreadPool] 线程启动--HS/HA模式" << std::endl;
    while(true){
        ...
        if(actor_mode_ == 0){       // Reactor模式：工作线程负责 I/O 和业务逻辑
            // 处理请求
            if(0 == task->io_state_){        // 读状态
        ...
        }else{          // Proactor模式（主线程已完成 I/O，工作线程只处理业务逻辑）
            // 按需获取数据库连接
            SQLConnectionRAII mysqlconn(&task->mysql_, sql_conn_pool_);
